...
module MusicArchitecture
  ...
  def self.show_panel
    ...
    @highlight_material.color = Sketchup::Color.new(100, 100, 255)
  ...
  def self.place_note(key)
    ...
    tr = Geom::Transformation.rotation(@current_reference_point, axis, angle.degrees)
  ...
  def self.change_direction(action)
    return unless @current_version_index
    group = @versions[@current_version_index]
    if action == "left" && group[:advance_dir] != "Z"
      if group[:advance_dir] == "X" && group[:base_spacing] > 0
        group[:advance_dir] = "Y"
        group[:base_spacing] *= -1
      elsif group[:advance_dir] == "Y" && group[:base_spacing] < 0
        group[:advance_dir] = "X"
        group[:base_spacing] *= -1
      end
    elsif action == "right" && group[:advance_dir] != "Z"
      if group[:advance_dir] == "X" && group[:base_spacing] > 0
        group[:advance_dir] = "Y"
      elsif group[:advance_dir] == "Y" && group[:base_spacing] > 0
        group[:advance_dir] = "X"
    ...
    @versions.each_with_index do |group, index|
      ...
      model.definitions.each do |defn|
        if defn.name.end_with?("_v#{group[:id]}")
          defn.instances.each { |inst| inst.material = material }
        end
      end
    end
  end
    ...
    <<~HTML
      ...
      <body>
        ...
        <form id="paramsForm">
          ...
          <button type="button" onclick="adjustHeight(1)">+1倍高度</button>
          <button type="button" onclick="adjustHeight(-1)">-1倍高度</button>
      ...
      <script>
        ...
        function adjustHeight(delta) { sketchup.adjustHeight(delta); }
