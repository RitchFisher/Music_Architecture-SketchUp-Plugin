...
module MusicArchitecture
  ...
  def self.show_panel
    if @dialog.nil? || !@dialog.visible?
      ...
      @dialog.add_action_callback("enableKeyboardControl") { |_| enable_keyboard_control }
      @dialog.add_action_callback("disableKeyboardControl") { |_| disable_keyboard_control }
      @dialog.show
    end
    # 创建高亮材质
  ...
  # 启用键盘控制
  def self.enable_keyboard_control
    @music_tool = MusicTool.new
    Sketchup.active_model.select_tool(@music_tool)
    UI.messagebox("键盘控制已启用！按 z, s, x, d, c, v, g, b, h, n, j, m 生成音符。")
  end

  # 停用键盘控制
  def self.disable_keyboard_control
    Sketchup.active_model.select_tool(nil)
    UI.messagebox("键盘控制已停用。")
  end
  ...
  class MusicTool
    def onKeyDown(key, repeat, flags, view)
      # 将按键代码转换为字符
      char = key.chr rescue nil
      char = char.downcase if char
      puts "按键触发: #{char}"  # 调试输出
      if MusicArchitecture::KEY_MAP.key?(char)
        MusicArchitecture.place_note(MusicArchitecture::KEY_MAP[char])
      end
    end
  end
  ...
  def self.place_note(key)
    ...
    @last_spacing = group[:base_spacing] * factor[@current_note]
    vector = case group[:advance_dir]
             when 'X' then [@last_spacing, 0, 0]
             when 'Y' then [0, @last_spacing, 0]
             when 'Z' then [0, 0, @last_spacing]
             end
  ...
  def self.get_html_content
    <<~HTML
      ...
      <body>
        ...
        <button onclick="enableKeyboardControl()">启用键盘控制</button>
        <button onclick="disableKeyboardControl()">停用键盘控制</button>
        <form id="paramsForm">
      ...
      <script>
        ...
        function enableKeyboardControl() { sketchup.enableKeyboardControl(); }
        function disableKeyboardControl() { sketchup.disableKeyboardControl(); }
