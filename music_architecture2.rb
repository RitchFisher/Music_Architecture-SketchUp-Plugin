...
module MusicArchitecture
  ...
  def self.set_parameters(params)
    ...
    @base_spacing = params['spacing'].to_f  # 更新间距
    @version += 1  # 每次设置参数时，版本号递增
  ...
  def self.place_note(key)
    ...
    component_name = "#{@current_note} v#{@version}"  # 使用最新版本的组件
  ...
  def self.advance_reference_point
    vector = case @advance_dir
             when 'X' then [@base_spacing, 0, 0]
             when 'Y' then [0, @base_spacing, 0]
             when 'Z' then [0, 0, @base_spacing]
             end
    @reference_point += vector
    update_reference_point_display  # 更新面板显示
  end
  ...
  # 更新面板中的参考点显示
  def self.update_reference_point_display
    if @dialog && @dialog.visible?
      @dialog.execute_script("document.getElementById('ref_x').value = '#{@reference_point.x}';")
      @dialog.execute_script("document.getElementById('ref_y').value = '#{@reference_point.y}';")
      @dialog.execute_script("document.getElementById('ref_z').value = '#{@reference_point.z}';")
    end
  end
    ...
    <<~HTML
      ...
      <body>
        ...
        <form id="paramsForm">
          ...
          <label>间距: <input type="text" id="spacing" value="#{@base_spacing}"></label><br>
