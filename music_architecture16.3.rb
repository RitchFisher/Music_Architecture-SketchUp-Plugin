...
module MusicArchitecture
  ...
  def self.show_panel
    ...
    draw_dashed_line_preview if @current_version_index
  end
  ...
  def self.switch_group(direction)
    ...
    unless ["Z", "Z-"].include?(group[:advance_dir])
      @dialog.execute_script("window.originalDirection = '#{group[:advance_dir]}';")
    end
  ...
  def self.change_direction(action)
    ...
    if group[:rotation_axis] == current_axis
      @saved_direction = group[:advance_dir]
      new_group[:advance_dir] = "Z"
      new_group[:rotation_axis] = "Z"
      @dialog.execute_script("window.originalDirection = '#{group[:advance_dir]}';")
      puts "方向切换到 Z：#{group[:advance_dir]} -> Z, 旋转轴：#{group[:rotation_axis]} -> Z, 保存方向：#{@saved_direction}"
    else
      dir_map = {
        "X+" => ["Y+", "Y-"],
        "Y+" => ["X-", "X+"],
  ...
  def self.set_group_direction(dir)
    ...
    unless ["Z", "Z-"].include?(dir)
      @saved_direction = dir
      @dialog.execute_script("window.originalDirection = '#{dir}';")
    end
    new_group[:advance_dir] = dir
    new_group[:reference_point] = @current_reference_point.to_a
    puts "设置方向：#{group[:advance_dir]} -> #{dir}, 组ID：#{new_group[:id]}, 保存方向：#{@saved_direction}"
    create_note_components(new_group)
    save_group_to_model(new_group, new_group[:id])
    @last_action = :direction
    update_panel_display
  ...
  def self.draw_dashed_line_preview
    return unless @current_version_index
    clear_dashed_line_preview
    group = @versions[@current_version_index]
    factor = { "FN" => 1.0, "HN" => 0.5, "QN" => 0.25, "EN" => 0.125, "SN" => 0.0625 }
    spacing = group[:base_spacing].abs * factor[@current_note]
    vector = case group[:advance_dir]
    ...
    model = Sketchup.active_model
    entities = model.active_entities
    dash_group = entities.add_group
    dash_group.name = "DashedLinePreview"
    dash_length = mm_to_inch(10.0)
    gap_length = mm_to_inch(5.0)
    total_length = start_point.distance(end_point)
    num_segments = (total_length / (dash_length + gap_length)).floor
    direction = vector.normalize
    current_pos = start_point
    num_segments.times do
      next_pos = current_pos + direction * dash_length
      entities.add_line(current_pos, next_pos) if current_pos.distance(next_pos) > 0.001
      current_pos = next_pos + direction * gap_length
    end
    material = model.materials["DashedLineMaterial"] || model.materials.add("DashedLineMaterial")
    material.color = Sketchup::Color.new(255, 0, 0)
    dash_group.material = material
    puts "绘制虚线预览：从 #{start_point} 到 #{end_point}"
  end

  def self.clear_dashed_line_preview
    model = Sketchup.active_model
    entities = model.active_entities
    entities.each do |entity|
      if entity.is_a?(Sketchup::Group) && entity.name == "DashedLinePreview"
  ...
  def self.delete_current_group
    ...
    update_panel_display
    draw_dashed_line_preview unless @versions.empty?
  end
  ...
  def self.load_groups_from_model
    ...
    @current_version_index = @versions.any? ? @versions.size - 1 : nil
      ...
      <script>
        ...
        function toggleZDirection() {
          ...
          if (currentDir === 'Z' || currentDir === 'Z-') {
            sketchup.setGroupDirection(window.originalDirection || 'X+');
          } else {
            window.originalDirection = currentDir;
        ...
        function changeDirection(action) {
          ...
          if (action === 'left' || action === 'right') {
            var rotationAxis = document.getElementById('rotation_axis').value;
            var currentAxis = (currentDir === 'X+' || currentDir === 'X-') ? 'X' :
                              (currentDir === 'Y+' || currentDir === 'Y-') ? 'Y' : 'Z';
            if (rotationAxis === currentAxis && currentDir !== 'Z' && currentDir !== 'Z-') {
              window.originalDirection = currentDir;
            }
          }
          console.log("方向切换: " + action + ", 保存方向: " + window.originalDirection);
          sketchup.changeDirection(action);
        }
        ...
        function setDirection(dir) {
          var currentDir = document.getElementById('advance_dir').value;
          if ((currentDir === 'Z' && dir === 'Z-') || (currentDir === 'Z-' && dir === 'Z')) {
            // 不更新 originalDirection
          } else {
            window.originalDirection = dir;
          }
          sketchup.setGroupDirection(dir);
        }
