...
module MusicArchitecture
  ...
  unless file_loaded?(__FILE__)
    ...
    menu.add_item('Show Music Architecture Panel (ENG)') { show_panel }
  ...
  def self.show_panel
    if @dialog.nil? || !@dialog.visible?
      ...
      @dialog.add_action_callback("toggleRotationAxis") { |_| toggle_rotation_axis }
      @dialog.show
    end
  ...
  def self.switch_group(direction)
    ...
    puts "切换组：ID=#{@versions[old_index][:id]} -> #{group[:id]}"
  ...
  def self.set_group_direction(dir)
    ...
    if dir == "Z"
      ...
      if current_axis == group[:rotation_axis]
        new_group[:rotation_axis] = "Z"
        puts "Z 轴同步：rotation_axis=#{group[:rotation_axis]} -> Z"
      end
    end
    
    puts "设置方向：#{group[:advance_dir]} -> #{dir}, 组ID：#{new_group[:id]}"
    create_note_components(new_group)
    save_group_to_model(new_group, new_group[:id])
    @last_action = :direction
    update_panel_display
  ...
  def self.update_preview
    model = Sketchup.active_model
    if @preview_entity
      model.entities.erase_entities(@preview_entity)
      @preview_entity = nil
    end
    return unless @current_version_index
    group = @versions[@current_version_index]
    ...
    next_point = @current_reference_point + vector
    @preview_entity = model.entities.add_cline(@current_reference_point, next_point)
    puts "预览位置：#{next_point}"
  end

  def self.toggle_rotation_axis
    return unless @current_version_index
    group = @versions[@current_version_index]
    advance_axis = case group[:advance_dir]
                   when "X+", "X-" then "X"
                   when "Y+", "Y-" then "Y"
                   when "Z", "Z-" then return  # Z 轴禁用
                   end
    if group[:rotation_axis] == advance_axis
      group[:rotation_axis] = "Z"
    elsif group[:rotation_axis] == "Z"
      group[:rotation_axis] = advance_axis
    else
      return  # 不在 advance_axis 或 Z，禁用
    end
    puts "旋转轴切换：#{advance_axis} <-> Z"
    create_note_components(group)
    save_group_to_model(group, group[:id])
    update_panel_display
    update_preview
  ...
  def self.get_html_content
    <<~HTML
      ...
      <body>
        ...
        <p>Current Group: <span id="group_id">Unselected</span></p>
        <button onclick="loadGroups()">Read Group Date</button>
        <button onclick="deleteCurrentGroup()">Delete current group</button>
        <form id="paramsForm">
          <label>Length (mm): <input type="number" id="length" value="3000"></label><br>
          <label>Width (mm): <input type="number" id="width" value="100"></label><br>
          <label>Height (mm): <input type="number" id="height" value="3000"></label><br>
          <label>Proceeding Direction:
          ...
          <label>Standard Spacing (mm): <input type="number" id="spacing" value="3000"></label><br>
          <button type="button" onclick="createNewGroup()">Create New Group</button>
          <button type="button" onclick="updateCurrentGroup()">Save to Current Group</button>
          <button type="button" onclick="syncSpacing()">Sync Spacing and Length</button>
          <button type="button" onclick="syncWidth()">Sync Width and Length</button>
          <button type="button" onclick="reverseSpacing()">Reverse Spacing</button>
          <button type="button" onclick="adjustDimension('length', 1)">+1 Length</button>
          <button type="button" onclick="adjustDimension('length', -1)">-1 Length</button>
          <button type="button" onclick="reduceHalfDimension('length')">Length</button>
          <button type="button" onclick="adjustDimension('width', 1)">+1Width</button>
          <button type="button" onclick="adjustDimension('width', -1)">-1Width</button>
          <button type="button" onclick="reduceHalfDimension('width')">Halves Width</button>
          <button type="button" onclick="adjustDimension('height', 1)">+1 Height</button>
          <button type="button" onclick="adjustDimension('height', -1)">-1 Height</button>
          <button type="button" onclick="reduceHalfDimension('height')">Halves Height</button>
          <button type="button" onclick="adjustDimension('base_spacing', 1)">+1 Spacing</button>
          <button type="button" onclick="adjustDimension('base_spacing', -1)">-1 Spacing</button>
          <button type="button" onclick="reduceHalfDimension('base_spacing')">Halves Spacing</button>
          <button type="button" onclick="changeDirection('left')">Turn Left</button>
          <button type="button" onclick="changeDirection('right')">Turn Right</button>
          <button type="button" onclick="undoLastPlacement()">Undo</button>
          <button type="button" onclick="toggleRotationAxis()">Switch Rotation Axis</button>
        </form>
        <h3>Current Position (mm)</h3>
        <p>X: <input type="number" id="ref_x" value="0"></p>
        <p>Y: <input type="number" id="ref_y" value="0"></p>
        <p>Z: <input type="number" id="ref_z" value="0"></p>
        <button onclick="updateReferencePoint()">Update Position</button>
        <button onclick="resetReferencePoint()">Reset Position</button>
        <button onclick="getReferencePoint()">Get Position</button>
        <button onclick="switchGroup('prev')">Prev Group</button>
        <button onclick="switchGroup('next')">Next Group</button>
        <h3>Note Placement</h3>
        ...
        <button onclick="setNoteType('1')">FN 1/1</button>
        <button onclick="setNoteType('2')">HN 1/2</button>
        <button onclick="setNoteType('3')">QN 1/4</button>
        <button onclick="setNoteType('4')">EN 1/8</button>
        <button onclick="setNoteType('5')">SN 1/16</button>
        <h3>Rest</h3>
        <button onclick="advanceReferencePoint()">Rest</button>
      </body>
      <script>
        ...
        document.addEventListener('keydown', function(e) {
          ...
          var validKeys = ['z', 's', 'x', 'd', 'c', 'v', 'g', 'b', 'h', 'n', 'j', 'm', '0', ' ', '1', '2', '3', '4', '5', 
                           'ArrowLeft', 'ArrowRight', 'ArrowUp', 'ArrowDown', '[', ']', '\\\\', 'l', ';', '\\'', ',', '.', '/', 'p', '-', '+', '=', 'o', 'Backspace', 'Control'];
          if (validKeys.includes(key) && e.target.tagName !== 'INPUT' && e.target.tagName !== 'TEXTAREA') {
            console.log("捕获按键: " + key + ", 代码: " + e.code);
            if (key === '0' || key === ' ') {
            ...
            } else if (key === 'Control' && e.code === 'ControlRight') {
              sketchup.toggleRotationAxis();
            } else {
        ...
        function toggleRotationAxis() { sketchup.toggleRotationAxis(); }
        function getParams() {
          return {
