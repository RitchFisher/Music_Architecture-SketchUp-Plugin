...
module MusicArchitecture
  ...
  def self.advance_reference_point
    ...
    prev_note = @placement_history.last&.[](:note_type)
    current_length = group[:length] * factor[@current_note]
    spacing = current_length / 2.0
    if prev_note
      prev_length = group[:length] * factor[prev_note]
      spacing += prev_length / 2.0
    end
    
    puts "间距计算：prev_note=#{prev_note || '无'}, prev_length=#{prev_note ? inch_to_mm(group[:length] * factor[prev_note]) : 0}, current_note=#{@current_note}, current_length=#{inch_to_mm(current_length)}, spacing=#{inch_to_mm(spacing)}"
  ...
  def self.undo_last_placement
    if @placement_history.any?
      ...
      factor = { "FN" => 1.0, "HN" => 0.5, "QN" => 0.25, "EN" => 0.125, "SN" => 0.0625 }
      prev_note = @placement_history.last&.[](:note_type)
      current_length = group[:length] * factor[last[:note_type]]
      spacing = current_length / 2.0
      if prev_note
        prev_length = group[:length] * factor[prev_note]
        spacing += prev_length / 2.0
      ...
      group[:reference_point] = @current_reference_point.to_a
      save_group_to_model(group, group[:id])
      puts "回退音符，恢复参考点：#{@current_reference_point}"
      update_panel_display
      update_preview
    end
  end
    ...
    <<~HTML
      ...
      <body>
        ...
        <form id="paramsForm">
          ...
          <button type="button" onclick="toggleRotationAxis()">改变旋转轴</button>
        </form>
        <h3>当前参考点 (mm)</h3>
      ...
      <script>
        ...
        document.addEventListener('keydown', function(e) {
          ...
          var validKeys = ['z', 's', 'x', 'd', 'c', 'v', 'g', 'b', 'h', 'n', 'j', 'm', '0', ' ', '1', '2', '3', '4', '5', 
                           'ArrowLeft', 'ArrowRight', 'ArrowUp', 'ArrowDown', 'p', ',', '[', ']', '\\\\', 'l', ';', '\\'', '.', '/', '-', '+', '=', 'o', 'Backspace', 'Control'];
