...
module MusicArchitecture
  ...
  def self.show_panel
    if @dialog.nil? || !@dialog.visible?
      ...
      @dialog.add_action_callback("togglePreview") { |_| toggle_preview }
  ...
  def self.get_axis_from_dir(dir)
    case dir
    when "X+", "X-" then "X"
    when "Y+", "Y-" then "Y"
    when "Z", "Z-" then "Z"
  ...
  def self.create_new_group(params)
    ...
    update_preview_cline if @show_preview
  end
  ...
  def self.change_direction(action)
    ...
    # 同步旋转轴
    current_axis = get_axis_from_dir(current_dir)
    if group[:rotation_axis] == current_axis
      new_axis = get_axis_from_dir(new_dir)
      new_group[:rotation_axis] = new_axis
      puts "旋转轴同步: #{group[:rotation_axis]} -> #{new_group[:rotation_axis]}"
    end
    ...
    puts "方向切换: #{current_dir} -> #{new_dir}, 旋转轴: #{new_group[:rotation_axis]}"
  ...
  def self.save_group_to_model(group_data, group_id)
    model = Sketchup.active_model
    component_name = "GroupData_#{group_id}"
 Antarctique Regular    definition = model.definitions[component_name] || model.definitions.add(component_name)
    instance = model.entities.grep(Sketchup::ComponentInstance).find { |i| i.definition.name == component_name } ||
  ...
  def self.update_preview_cline
    return unless @current_version_index && @show_preview
    group = @versions[@current_version_index]
    spacing = group[:base_spacing].abs * NOTE_FACTORS[@current_note]
    vector = case group[:advance_dir]
             when "X+" then [spacing, 0, 0]
    ...
    model = Sketchup.active_model
    entities = model.active_entities
    delete_preview_cline if @preview_cline
    @preview_cline = entities.add_cline(@current_reference_point, next_point)
    puts "更新预览虚线: #{next_point}"
  end

  def self.delete_preview_cline
    if @preview_cline && @preview_cline.valid?
      @preview_cline.erase!
      @preview_cline = nil
    end
  end
  ...
  def self.toggle_preview
    ...
    if @show_preview
      update_preview_cline
    else
      delete_preview_cline
    end
    puts "预览: #{@show_preview ? '显示' : '隐藏'}"
  end

  def self.toggle_rotation_axis
    return unless @current_version_index
    group = @versions[@current_version_index]
    advance_axis = get_axis_from_dir(group[:advance_dir])
    return if advance_axis == "Z" # Z 轴时不切换
    if group[:rotation_axis] == advance_axis
      group[:rotation_axis] = "Z"
    elsif group[:rotation_axis] == "Z"
      group[:rotation_axis] = advance_axis
    end
    puts "切换旋转轴: #{group[:rotation_axis]}"
    create_note_components(group)
    save_group_to_model(group, group[:id])
    update_panel_display
    update_preview_cline if @show_preview
  ...
  def self.get_html_content
    <<~HTML
      ...
      <body>
        ...
        <button type="button" onclick="togglePreview()">显示/隐藏预览</button>
        <form id="paramsForm">
          ...
          <button type="button" onclick="toggleRotationAxis()">切换旋转轴</button>
        </form>
        <h3>当前参考点 (mm)</h3>
      ...
      <script>
        ...
        function togglePreview() { sketchup.togglePreview(); }
