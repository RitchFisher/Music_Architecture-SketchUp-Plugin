...
module MusicArchitecture
  ...
  def self.create_new_group(params)
    ...
    preview_next_position # 创建新组后显示虚线
  end
  ...
  def self.switch_group(direction)
    ...
    update_panel_display
    preview_next_position # 切换组后显示虚线
  end
  ...
  def self.set_reference_point(x, y, z)
    ...
    save_group_to_model(group, group[:id])
    update_panel_display
    preview_next_position # 更新参考点后显示虚线
  end
  ...
  def self.create_guide_line(point1, point2)
    model = Sketchup.active_model
    entities = model.active_entities
    @guide_line.erase! if @guide_line && @guide_line.valid?
    @guide_line = entities.add_cline(point1, point2)
    puts "创建虚线: #{point1} -> #{point2}"
  end

  def self.clear_guide_line
    @guide_line.erase! if @guide_line && @guide_line.valid?
    @guide_line = nil
  end

  def self.preview_next_position
    return unless @current_version_index
    group = @versions[@current_version_index]
    ...
    create_guide_line(@current_reference_point, next_point)
  end

  def self.place_note(key)
    return unless @current_version_index
    preview_next_position # 先显示虚线预览下一个位置
    group = @versions[@current_version_index]
    note_map = {
  ...
  def self.set_note_type(key)
    ...
    preview_next_position # 更改音符类型后更新虚线
  end
  ...
  def self.change_direction(action)
    ...
    update_panel_display
    preview_next_position # 方向切换后更新虚线
  end
  ...
  def self.set_group_direction(dir)
    ...
    @last_action = :direction
    update_panel_display
    preview_next_position # 设置方向后更新虚线
  end
  ...
  def self.get_reference_point_from_selection
    ...
    save_group_to_model(group, group[:id])
    update_panel_display
    preview_next_position # 获取参考点后显示虚线
  end
  ...
  def self.delete_current_group
    ...
    update_panel_display
    preview_next_position if @current_version_index # 删除后如果有组则显示虚线
  end
  ...
  def self.load_groups_from_model
    ...
    preview_next_position if @current_version_index # 加载后显示虚线
  end
