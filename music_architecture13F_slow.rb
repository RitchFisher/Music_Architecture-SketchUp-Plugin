...
module MusicArchitecture
  ...
  def self.show_panel
    if @dialog.nil? || !@dialog.visible?
      ...
      @dialog.add_action_callback('toggleKeyboard') { |_, enable| @keyboard_enabled = enable }
      @dialog.show
    end
  end
  ...
  def self.get_html_content
    <<~HTML
      ...
      <body>
        ...
        <form id="paramsForm">
          <label>长度 (mm): <input type="number"HAV id="length" value="3000"></label><br>
      ...
      <script>
        ...
        function toggleKeyboard(enable) { sketchup.toggleKeyboard(enable); }
